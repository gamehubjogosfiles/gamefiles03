...
Stick.prototype.handleInput = function (delta) {
    ...
    else if (this.power>0 && (Mouse.left.down || Touch._active)){
      var mobilePower = Touch.checkShoot();
      if (mobilePower > 0) {
          this.power = mobilePower;
          this.origin.x = 970 + (this.power * 1.6); // Ajustar visualmente o taco para trás
      } else if (Touch._active) {
          // No mobile, se não disparou pelo botão, mantemos a força da barra
          this.power = Touch._power;
          this.origin.x = 970 + (this.power * 1.6);
          return; // Esperar o botão "GO"
      }
